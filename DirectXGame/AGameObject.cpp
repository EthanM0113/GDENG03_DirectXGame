...
void AGameObject::recomputeMatrix(float matrix[16])
{
	...
	setPosition(Vector3D(localMatrix.getTranslation().getValues().x, localMatrix.getTranslation().getValues().y, localMatrix.getTranslation().getValues().z));
	*/
}
...
{
	// not too sure how to do this, review previous recursive code
	ComponentList tempList;
